...
*┃❍ ᴠᴇʀsɪᴏɴ :* v${version}
*┃❍ ᴜsᴇʀ    :* ${userName}
*┃❍ ᴘᴀɪʀ    :* https://kaisen-free-deploy.vercel.app
*╰═════════════════⊷*
${readMore}
*♡︎•━━━━━━☻︎━━━━━━•♡︎*
`;
    ...
    for (const command of commands) {
      const cmd = command.pattern
        ?.toString()
        .match(/(\W*)([A-Za-züşiğöç1234567890]*)/)?.[2];
    ...
    const [typFont, ptrnFont] = BOT_INFO_FONT.split(";").map((f) =>
      isNaN(f) || parseInt(f) > 35 ? null : f
    );
    ...
    try {
      if (mediaUrl && isUrls(mediaUrl)) {
        ...
        await message.client.sendMessage(message.jid, opts, {
          quoted: message,
        });
      } else {
