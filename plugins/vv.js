...
plugin(
  ...
  async (message) => {
    ...
    if (!reply)
      return await message.send("Reply to a view-once image/video/audio.");
    let raw = reply.message;
    while (raw?.ephemeralMessage || raw?.viewOnceMessage) {
      raw =
        raw.ephemeralMessage?.message || raw.viewOnceMessage?.message || raw;
    }
    ...
    try {
      ...
      if (
        ...
        return await message.send(
          "⚠️ This message does not contain downloadable media."
        );
      }
      ...
      const media = fs.readFileSync(tempFile);
      if (!botNumber)
        return await message.send("⚠️ Failed to detect bot number.");
