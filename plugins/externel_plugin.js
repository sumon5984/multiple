...
plugin(
  ...
  async (message, match) => {
    ...
    if (match && extractUrlsFromString(match)) {
      ...
      if (!urll[0])
        return message.send("```invalid url```", {
          linkPreview: linkPreview(),
        });
      urll.map(async (url) => {
        let NewUrl = !url?.toString().includes("/raw")
          ? url.toString()
          : url.toString().split("/raw")[0];
        let plugin_name;
        let { data, status } = await axios(NewUrl + "/raw").catch((e) => {
          return message.send("```url must be a valid url or gist url```", {
            linkPreview: linkPreview(),
          });
        });
        if (status == 200) {
          try {
            plugin_name = data
              ...
              .replace(/"/g, "");
            fs.writeFileSync(
              __dirname + "/" + plugin_name.split(",")[0] + ".js",
              data
            );
          ...
          await message.send(`_newly installed plugins are *${plugin_name}*_`, {
            linkPreview: linkPreview(),
          });
          await personalDB(
            ["plugins"],
