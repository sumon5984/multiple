...
async function isBotAdmin(m, conn) {
  try {
    // Check if it's a group chat
    if (!m.key?.remoteJid?.endsWith("@g.us") && !m.jid?.endsWith("@g.us")) {
      return false;
    }

    const client = conn || m.client;
    if (!client || !client.user) return false;

    const groupJid = m.key?.remoteJid || m.jid;

    // Get group metadata
    const groupMetadata = await client.groupMetadata(groupJid);
    if (!groupMetadata || !groupMetadata.participants) return false;

    // Normalize bot JIDs - check both user.id and user.lid
    const botJidFromId = jidNormalizedUser(client.user.id);
    const botJidFromLid = jidNormalizedUser(client.user.lid);

    // Find bot participant by checking both IDs
    let botParticipant = groupMetadata.participants.find((p) => {
      const participantJid = jidNormalizedUser(p.id);
      return (
        participantJid === botJidFromId ||
        (botJidFromLid && participantJid === botJidFromLid)
      );
    });

    // If still not found, try alternative JID formats
    if (!botParticipant) {
      const alternativeJids = [
        client.user.id,
        client.user.lid,
        client.user.id.split(":")[0] + "@s.whatsapp.net",
        client.user.lid
          ? client.user.lid.split(":")[0] + "@s.whatsapp.net"
          : null,
      ].filter(Boolean);

      botParticipant = groupMetadata.participants.find((p) =>
        alternativeJids.some(
          (jid) => jidNormalizedUser(p.id) === jidNormalizedUser(jid)
        )
      );
    }

    // Check if bot is admin or superadmin
    return (
...
/**
 * Checks if the sender is an admin in the group
 * @param {object} m - The Baileys message object
 * @param {object} conn - Baileys connection (optional, will use m.client if not provided)
...
async function isAdmin(m, conn) {
  ...
  const client = conn || m.client;
  if (!client) {
    console.log("No client available - returning false");
    return false;
  }
  ...
  try {
    ...
    const sender = groupMetadata.participants.find(
      (p) => jidNormalizedUser(p.id) === senderJid
    );

    const result = sender?.admin === "admin" || sender?.admin === "superadmin";
...
function isBot(message) {
  try {
    ...
    // Normalize bot and sender IDs
    const botId = jidNormalizedUser(message.client.user.id || "");
    const botLid = message.client.user.lid
      ? jidNormalizedUser(message.client.user.lid)
      : null;
    const senderId = jidNormalizedUser(message.sender || "");

    // Check against known identifiers
    if (message.fromMe) return true; // Baileys auto-flags messages sent by bot
    if (senderId === botId) return true;
    if (botLid && senderId === botLid) return true;

    return false;
  } catch (err) {
    console.error("isBot() error:", err);
    return false;
  }
}

async function isAccess(message) {
  // Bot owner always has access
  if (await isBot(message)) {
    return true;
  }

  // Check if sender is actually the bot itself (to prevent bot loops)
  const botJid = jidNormalizedUser(message.client.user.id);
  const botLid = jidNormalizedUser(message.client.user.lid);
  const senderJid = jidNormalizedUser(message.sender);

  if (senderJid === botJid || senderJid === botLid) {
    return false; // Deny if bot is talking to itself
  }

  // For users in groups, check if they're admin
  if (message.isGroup) {
    try {
      return await isAdmin(message);
    } catch (error) {
  ...
  // For users in DMs, deny access (only owner and admins allowed)
  return false;
}
