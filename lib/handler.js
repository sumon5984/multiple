...
async function isBotAdmin(m, conn) {
  try {
    ...
    const botJidFromLid = client.user.lid
...
async function isAccess(message) {
  ...
  const isOwner = message.fromMe || message.itsMe;
  if (isOwner) {
    return true;
  }
  ...
  if (message.isGroup) {
    try {
      const adminResult = await isAdmin(message);
      if (adminResult) {
        return true;
      }
    } catch (error) {
...
async function isBot(message) {
  // Only check if the user is the bot owner
  return message.fromMe || message.itsMe;
}
