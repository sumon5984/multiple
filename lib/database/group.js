  ...
  if (method === 'set') {
    ...
    if (!dbData) {
      ...
      if (options.jid === 'global') {
        if (!createData.global_welcome) {
          createData.global_welcome = JSON.stringify({
            status: 'true',
            all_status: 'true',
            message: 'Hey &mention welcome to &name all groups members &size &pp'
          });
        }
        if (!createData.global_exit) {
          createData.global_exit = JSON.stringify({
            status: 'true',
            all_status: 'true',
            message: 'Goodbye &mention! Thanks for being part of &name &pp'
          });
        }
        if (!createData.global_pdm) {
          createData.global_pdm = JSON.stringify({
            status: 'true',
            all_status: 'true'
          });
        }
  ...
  if (method === 'add') {
    ...
    if (filter[type] === 'object') {
      ...
      if (dbData) {
        ...
        const createData = { jid: options.jid, [type]: options.content };
