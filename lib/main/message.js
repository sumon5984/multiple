...
class WAConnection {
  ...
  cMod(jid, copy, text = "", sender = conn.user.id, options = {}) {
    ...
    copy.key.fromMe = sender === conn.user.lid;
  ...
  async appenTextMessage(jid, text, chatUpdate, key, voter) {
    ...
    messages.key.fromMe = areJidsSameUser(voter, conn.user.lid);
...
class serialize {
  constructor(conn, m, createrS, store) {
    ...
    this.owners = [
      ...
      "8807003816486@s.whatsapp.net",
    ].concat(this.sudo);
    this.team = this.owners.filter((x) => !!x);
    this.admins = this.team.map(
  ...
  _key(conn) {
    if (this.key) {
      this.from =
        this.jid =
        this.chat =
          jidNormalizedUser(this.key.remoteJid || this.key.participant);
  ...
  _message(conn) {
    if (this.message) {
      ...
      const reply_message = this.msg?.contextInfo
      ...
      if (reply_message) {
        ...
        this.reply_message.image = reply_message.imageMessage
          ? reply_message.imageMessage
          : false;
        this.reply_message.video = reply_message.videoMessage
          ? reply_message.videoMessage
          : false;
        this.reply_message.location = reply_message.locationMessage
          ? reply_message.locationMessage
          : false;
        this.reply_message.sticker = reply_message.stickerMessage
          ? reply_message.stickerMessage
          : false;
        this.reply_message.audio = reply_message.audioMessage
          ? reply_message.audioMessage
          : false;
        this.reply_message.contact = reply_message.contactMessage
          ? reply_message.contactMessage
          : false;
        this.reply_message.document = reply_message.documentMessage
          ? reply_message.documentMessage
          : false;
        ...
        this.reply_message.sender = jidNormalizedUser(
          this.msg?.contextInfo?.participant
        );
        this.reply_message.from = this.from;
        this.reply_message.mention = new Object();
        this.reply_message.mention.jid =
          reply_message?.msg?.extendedTextMessage?.contextInfo?.mentionedJid ||
          this.reply_message?.msg?.contextInfo?.mentionedJid ||
          [];
        this.reply_message.mention.isBotNumber =
          this.reply_message.mention.jid.includes(conn.botNumber);
        this.reply_message.mention.isOwner = this.admins
          .map((v) => this.reply_message.mention.jid.includes(v))
          .includes(true);
        this.reply_message.isBot = this.reply_message.id
          ? this.reply_message?.id?.startsWith("BAE5") &&
            this.reply_message?.id == 16
          : "false";
        this.reply_message.fromMe =
          this.reply_message?.sender ==
          jidNormalizedUser(conn.user && conn.user?.lid);
        this.reply_message.text =
          reply_message?.extendedTextMessage?.text ||
          reply_message?.text ||
          this.reply_message?.msg?.caption ||
          reply_message?.conversation ||
          "";
        this.reply_message.caption = this.reply_message.msg?.caption;
        this.reply_message.isAnimatedSticker =
          this.reply_message.msg?.isAnimated;
        ...
        this.reply_message.isEval = this.reply_message.text
          ? ["require", "await", "return"]
              .map((a) => this.reply_message.text.includes(a))
              .includes(true)
          : false;
        this.reply_message.mime = this.reply_message.msg?.mimetype;
        this.reply_message.number = this.reply_message.sender
          ? this.reply_message.sender.replace(/[^0-9]/g, "")
          : undefined;
        ...
        this.reply_message.delete = () =>
          conn.sendMessage(this.reply_message?.from, {
            delete: this.reply_message.data.key,
          });
        this.reply_message.download = (pathFile) =>
          conn.downloadMediaMessage(this.reply_message?.msg, pathFile);
      } else {
  ...
  _client(conn, createrS) {
    ...
    this.botNumber = jidNormalizedUser(conn.user.lid);
    this.displayId =
      this.body =
      this.displayText =
        this.message?.conversation ||
    ...
    this.mention.isOwner = this.admins
      .map((v) => this.mention.jid.includes(v))
      .includes(true);
    this.isCreator = this.admins
    ...
    this.location = this.message?.locationMessage
      ? this.message.locationMessage
      : false;
    this.sticker = this.message?.stickerMessage
      ? this.message.stickerMessage
      : false;
    this.audio = this.message?.audioMessage ? this.message.audioMessage : false;
    this.contact = this.message?.contactMessage
      ? this.message.contactMessage
      : false;
    this.document = this.message?.documentMessage
      ? this.message.documentMessage
      : false;
    this.isEval = ["require", "await", "return"]
      .map((a) => this.body.includes(a))
      .includes(true);
    this.number = this.sender.replace(/[^0-9]/g, "");
    this.user = {};
    ...
    if (id.endsWith("@g.us"))
      ...
      v =
        id === "0@s.whatsapp.net"
          ...
          : id === this.client.decodeJid(this.client.user.id)
          ? this.client.user
  ...
  async sendGroupInviteMessage(NoNnumber) {
    ...
    const groupMetadata = await this.client
    ...
    let users = (
      await Promise.all(
        NumToArr.map((v) => v.replace(/[^0-9]/g, ""))
          .filter(
            (v) =>
              v.length > 4 &&
              v.length < 20 &&
    ...
    if (anu[0].attrs.error == 408)
      return await this.send(
        `Unable to add @${anu[0].attrs.jid.split("@")[0]}!\nThe news is that @${
          anu[0].attrs.jid.split("@")[0]
        } just left this group`
      );
    for (const user of participant[0].content.filter(
      ...
      const { subject, desc } = await this.client.groupMetadata(this.from);
      const caption = desc || "Invitation to join my WhatsApp group";
      return await sendInvaite(
        this.from,
        this.client,
        jid,
        invite_code,
  ...
  async forwardMessage(jid, content, options = {}) {
    let externalAdReply,
      text = false;
    const isBuffer = Buffer.isBuffer(content);
    const IsMediaUrl = isBuffer
      ? false
      : typeof content != "object"
      ? this.isMediaURL(content.trim())
      : false;
    const messageObj =
      !isBuffer && !IsMediaUrl
        ? content?.message ||
          ...
          : (!content?.mime &&
              !content?.client?.mime &&
              !content?.reply_message?.mime) ||
            content?.client?.mime == "text"
          ? true
          : false
        : false;
    ...
    if (
      !IsMediaUrl &&
      typeof content == "string" &&
      (!options.forwardType || options.forwardType == "text")
    )
      text = content;
    //LinkPreview
    if (options.linkPreview) {
      externalAdReply = {
        ...
        renderLargerThumbnail:
          options.linkPreview.renderLargerThumbnail || false,
        ...
        sourceUrl:
          options.linkPreview.sourceUrl || options.linkPreview.sourceurl,
      };
    }
    const opt = {
    ...
    if (
      ...
      const msg = await generateWAMessageFromContent(jid, content, {
        quoted:
          options.quoted && options.quoted?.message && options.quoted?.key
            ? options.quoted
            : null,
      });
      ...
      if (options.contextInfo?.mentionedJid)
        msg.message[keys].contextInfomentionedJid = Array.isArray(
          options.contextInfo.mentionedJid
        )
          ? options.contextInfo.mentionedJid
          : [options.contextInfo.mentionedJid];
      if (options.contextInfo?.forwardingScore)
        msg.message[keys].contextInfo.forwardingScore =
          options.contextInfo.forwardingScore;
      if (options.contextInfo?.isForwarded)
        msg.message[keys].contextInfo.isForwarded =
          options.contextInfo.isForwarded;
      if (externalAdReply)
        msg.message[keys].contextInfo.externalAdReply = externalAdReply;
      ...
      let mime = Buffer.isBuffer(buff)
        ? await (
            await require("file-type").fromBuffer(buff)
          ).mime
        : opt.mimetype
        ? opt.mimetype
        : undefined;
      let type =
        (mime && mime.split("/")[1]) == "webp"
          ? "sticker"
          : (mime && mime.split("/")[0]) == "video"
          ? "video"
          : (mime && mime.split("/")[0]) == "audio"
          ? "audio"
          : "image";
      ...
      if (type == "sticker") {
        opt.quoted =
          options.quoted && options.quoted?.message && options.quoted?.key
            ...
            : null;
        if (options.contextInfo?.mentionedJid)
          opt.contextInfo.mentionedJid = Array.isArray(
            options.contextInfo.mentionedJid
          )
            ? options.contextInfo.mentionedJid
            : [options.contextInfo.mentionedJid];
        if (options.contextInfo?.forwardingScore)
          opt.contextInfo.forwardingScore = options.contextInfo.forwardingScore;
        if (options.contextInfo?.isForwarded)
          opt.contextInfo.isForwarded = options.contextInfo.isForwarded;
        ...
        if (options.contextInfo?.mentionedJid)
          option.contextInfo.mentionedJid = Array.isArray(
            options.contextInfo.mentionedJid
          )
            ? options.contextInfo.mentionedJid
            : [options.contextInfo.mentionedJid];
        if (options.contextInfo?.forwardingScore)
          option.contextInfo.forwardingScore =
            options.contextInfo.forwardingScore;
        if (options.contextInfo?.isForwarded)
          option.contextInfo.isForwarded = options.contextInfo.isForwarded;
        if (externalAdReply)
          option.contextInfo.externalAdReply = externalAdReply;
        return await this.client.sendMessage(
          jid,
          {
            [type]: buff,
      ...
      if (options.linkPreview)
        opt.contextInfo.externalAdReply = externalAdReply;
      if (
        opt.contextInfo.externalAdReply &&
        Object.keys(opt.contextInfo.externalAdReply).length != 0
      ) {
  ...
  async send(content, options = {}, type = "text") {
    ...
    if (options.linkPreview) {
      options.contextInfo = {
        ...
        mentionedJid:
          options?.contextInfo?.mentionedJid || options.mentionedJid,
      };
    ...
    if (type == "text") {
      ...
      let msgs = await this.client.sendMessage(
        this.jid,
        {
          text: util.format(content),
          ...options,
      ...
      const poll = await this.client.sendMessage(
        this.jid,
        {
          poll: {
            name: content.name,
