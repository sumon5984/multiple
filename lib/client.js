...
const handleChatbot = require('./chatbot');
  conn.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // =============================================================================
    // EVENT HANDLERS (for your main bot file)
    // =============================================================================

    // Enhanced welcome event handler
    conn.ev.on('group-participants.update', async (update) => {
      ...
      try {
        ...
        // Get group-specific and global settings
        const groupData = await groupDB(['welcome'], { jid: groupJid, content: {} }, 'get');
        const globalData = await groupDB(['global_welcome'], { jid: 'global', content: {} }, 'get');

        const groupWelcome = groupData?.welcome || { status: 'false' };
        const globalWelcome = globalData?.global_welcome || { status: 'true', all_status: 'true' };

        // Check if welcome should be sent - priority logic
        const globalAllEnabled = globalWelcome.all_status === 'true';
        const groupEnabled = groupWelcome.status === 'true';

        let shouldSend = false;
        let messageToUse = '';

        if (globalAllEnabled) {
          // Global "all" setting is enabled - use it
          shouldSend = true;
          messageToUse = globalWelcome.message || 'Hey &mention welcome to &name all groups members &size &pp';
        } else if (groupEnabled) {
          // Fall back to individual group setting
          shouldSend = true;
          messageToUse = groupWelcome.message || 'Hey &mention welcome to &name all groups members &size &pp';
        }
        ...
        for (const user of participants) {
          ...
          // Get user profile pic with improved error handling
          ...
          try {
            ...
            console.log(`⚠️ Could not fetch profile picture for ${user}: ${error.message}`);
            // Don't use external fallback that can be rate limited
            hasValidImage = false;
          }

          // Replace placeholders
          let text = messageToUse
            .replace(/&mention/g, mentionTag)
            .replace(/&size/g, groupSize)
            .replace(/&name/g, groupName);

          // Send welcome message with better error handling
          try {
            if (messageToUse.includes('&pp') && hasValidImage) {
              // Send with image only if we have a valid profile picture
              text = text.replace(/&pp/g, ''); // Remove &pp from text
              ...
              text = text.replace(/&pp/g, ''); // Remove &pp placeholder
              await conn.sendMessage(groupJid, {
                text,
                mentions: [user],
              });
            ...
            console.error(`❌ Failed to send welcome message for ${user}:`, messageError.message);
            // Try sending just a simple text message as last resort
            try {
              await conn.sendMessage(groupJid, {
                text: `Hey ${mentionTag} welcome to ${groupName}!`,
                mentions: [user],
              });
            } catch (fallbackError) {
              console.error(`❌ Even fallback message failed for ${user}:`, fallbackError.message);
            }
          }
        }
      } catch (error) {
    ...
    // Enhanced goodbye event handler
    ...
    conn.ev.on('group-participants.update', async (update) => {
      ...
      try {
        ...
        // Get group-specific and global settings
        const groupData = await groupDB(['exit'], { jid: groupJid, content: {} }, 'get');
        const globalData = await groupDB(['global_exit'], { jid: 'global', content: {} }, 'get');

        const groupExit = groupData?.exit || { status: 'false' };
        const globalExit = globalData?.global_exit || { status: 'true', all_status: 'true' };

        // Check if goodbye should be sent - priority logic
        const globalAllEnabled = globalExit.all_status === 'true';
        const groupEnabled = groupExit.status === 'true';
        ...
        if (globalAllEnabled) {
          // Global "all" setting is enabled - use it
          shouldSend = true;
          messageToUse = globalExit.message || 'Goodbye &mention! Thanks for being part of &name &pp';
        } else if (groupEnabled) {
          // Fall back to individual group setting
          shouldSend = true;
          messageToUse = groupExit.message || 'Goodbye &mention! Thanks for being part of &name &pp';
        }
...
//=================================================================================
        ...
        if (set_of_filters.has(chatUpdate.messages[0].key.id)) {
            ...
            // Get global PDM settings
            const { global_pdm } = await groupDB(['global_pdm'], { jid: 'global', content: {} }, 'get') || {};
            ...
            if (chatUpdate.messages[0]?.messageStubType && shutoff != 'true') {
              ...
              // Check if PDM should be active
              const globalAllEnabled = global_pdm?.all_status === 'true';
              const groupEnabled = pdm === 'true';

              // Priority: if "all" is enabled, use global settings; otherwise use group settings
              const shouldShowPDM = globalAllEnabled || groupEnabled;
