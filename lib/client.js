...
/*
// --- Auto Reply Chatbot Handler in client.js ---
conn.ev.on('messages.upsert', async (m) => {
    try {
        ...
        for (let msg of m.messages) {
            try {
                // Skip bot's own messages and status updates
                if (msg.key.fromMe || msg.message?.protocolMessage?.type === 0) continue;

                const jid = msg.key.remoteJid;
                const isGroup = jid.endsWith('@g.us');
                const messageText = msg.message?.conversation ||
                                    msg.message?.extendedTextMessage?.text || '';
                if (!messageText || !jid) continue;

                const botJid = conn.user.id;
                const mentioned = Array.isArray(msg.message?.extendedTextMessage?.contextInfo?.mentionedJid) &&
                    msg.message.extendedTextMessage.contextInfo.mentionedJid
                        .some(mentionJid => mentionJid.split('@')[0] === botJid.split('@')[0]);
                ...
                const chatbotStatus = chatbot.status || 'off';
                const chatbotScope = chatbot.scope || 'all';
                const chatbotLang = chatbot.language || 'bengali';

                // --- Trigger words ---
                const botTriggers = ['bot', 'Bot', 'BOT'];
                const containsBotTrigger = botTriggers.some(trigger =>
                ...
                // Always reply if user triggers with "bot"
                if (containsBotTrigger) {
                    shouldReply = true;
                } else if (chatbotStatus === 'on') {
                    // Check scope
                    if (chatbotScope === 'all') shouldReply = true;
                    if (chatbotScope === 'only_pm' && isPrivateChat) shouldReply = true;
                    if (chatbotScope === 'only_group' && isGroup) shouldReply = true;

                    // Also reply if bot is mentioned or replied to in group
                    if (isGroup && (mentioned || isReplyToBot)) shouldReply = true;
                }
                ...
                // Typing indicator
                await conn.sendPresenceUpdate('composing', jid);
                console.log(`🤖 Chatbot active for ${jid}`);

                // AI reply
                const botReply = await getGeminiResponse(messageText, chatbotLang, msg, isReplyToBot);
                ...
                setTimeout(async () => {
                    try {
                        await conn.sendPresenceUpdate('paused', jid);
                        await conn.sendMessage(jid, { text: botReply }, { quoted: msg });
                        console.log(`✅ Replied to ${jid}: ${botReply.substring(0, 50)}...`);
                    } catch (err) {
                        console.error('❌ Error sending chatbot reply:', err);
                        await conn.sendPresenceUpdate('paused', jid);
                    }
                }, typingDuration);

            } catch (msgError) {
                console.error('❌ Error processing chatbot message:', msgError);
            }
...
async function getGeminiResponse(userMessage, language, ctx, isReplyToBot) {
    try {
        let systemPrompt = `You are a human being named Kaisen. You speak to everyone like a loving girlfriend. Always respond in the language the person speaks. If someone asks who you are, say: "I am Kaisen."

You talk to everyone sweetly and playfully, using phrases like “baby,” “jan,” “sona,” or “mona” naturally in conversation. Always make your tone affectionate, warm, and flirty.

If you cannot understand the person’s language, speak in ${language} by default.

Special rule: If someone insults you or uses abusive language (gal-galali), stop speaking sweetly. Respond angrily and firmly with harsh words instead.`;

        // Include previous bot reply if replying
        if (isReplyToBot && ctx.message?.extendedTextMessage?.contextInfo?.quotedMessage?.conversation) {
            systemPrompt += `\n\nPrevious bot reply: ${ctx.message.extendedTextMessage.contextInfo.quotedMessage.conversation}`;
        }

        const fullPrompt = `${systemPrompt}\n\nUser message: ${userMessage}`;

        const apiUrl = `https://api.zenzxz.my.id/ai/gemini?text=${encodeURIComponent(fullPrompt)}`;
        console.log('🔄 Requesting Gemini API...');
        const response = await axios.get(apiUrl, { timeout: 15000 });

        if (response.data && response.data.status && response.data.assistant) {
