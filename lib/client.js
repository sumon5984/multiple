...
`;
  ...
  await conn.sendMessage(conn.user.id, {
        text: `🍉 Connecting to WhatsApp ${botNumber}`
      });
  }
    ...
    try {
        ...
        for (let msg of m.messages) {
            try {
                ...
                // --- If chatbot is off, only react to trigger words ---
                if (chatbotStatus !== 'on') {
                    ...
                    if (!containsBotTrigger) continue;

                    const responses = {
                        bengali: [
                            'কি হয়েছে আমাকে ডাকছো কেন🥺',
                            'কি হয়েছে সোনা আমাকে ডাকছো কেন🥰',
                            'বলো baby কি হয়েছে ❤️‍🩹',
                            'হ্যাঁ বলো, কি দরকার? 💕',
                            'আমি তো এখানেই আছি, কি হয়েছে? 😊'
                        ],
                        english: [
                            'Yes? What happened? 🥺',
                            'Why are you calling me sweetie? 🥰',
                            'Tell me baby, what happened? ❤️‍🩹',
                            'I\'m here, what do you need? 💕',
                            'What\'s up? 😊'
                        ]
                    };

                    const replyText = responses[chatbotLang][Math.floor(Math.random() * responses[chatbotLang].length)];
                    return await conn.sendMessage(jid, { text: replyText }, { quoted: msg });
                }

                // --- Scope check ---
                if (chatbotScope === 'only_pm' && isGroup) continue;
                if (chatbotScope === 'only_group' && !isGroup) continue;
                ...
                if (isPrivateChat) {
                    shouldReply = true; // Always reply in PM
                } else if (isGroup) {
                    shouldReply = mentioned || isReplyToBot || containsBotTrigger;
                }
                ...
                // AI reply
                const botReply = await getGeminiResponse(messageText, chatbotLang);
...
async function getGeminiResponse(userMessage, language = 'bengali') {
    try {
        const prompts = {
            bengali: 'আপনি একজন মানুষ, আপনার নাম কাইসেন। ...',
            english: 'You are a human being, your name is Kaisen. ...'
        };

        const systemPrompt = prompts[language] || prompts['bengali'];
        const fullPrompt = `${systemPrompt} User message: ${userMessage}`;
        ...
        const fallback = {
            bengali: [
                'আমি এখন একটু ব্যস্ত, পরে কথা বলবো 💕',
                'সরি, আমার মাথা একটু খারাপ লাগছে 😅',
                'আমার ইন্টারনেট কানেকশনে সমস্যা হচ্ছে 🥺',
                'একটু পরে কথা বলি, এখন একটু কাজ আছে 💖'
            ],
            english: [
        ...
        return (fallback[language] || fallback['bengali'])[Math.floor(Math.random() * 4)];
    }
}
