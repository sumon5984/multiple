...
const handleChatbot = require('./chatbot');
  conn.ev.on('messages.upsert', async ({ messages }) => {
    ...
    conn.ev.on('group-participants.update', async (update) => {
      ...
      try {
          console.log(`New participants added to ${groupJid}:`, participants);
          ...
          console.log(`Should send welcome: ${shouldSend}, Message: ${messageToUse}`);
          ...
          for (const user of participants) {
              try {
                  ...
                  // Send text + external preview with profile picture
                  ...
                  console.log(`Welcome sent successfully to ${user}`);
              } catch (userError) {
                  console.error(`Error sending welcome to ${user}:`, userError);
              }
          }
      } catch (error) {
        ...
        try {
            ...
            for (const user of participants) {
                ...
                let text = messageToUse
                    ...
                    .replace(/&name/g, groupName)
                    .replace(/&size/g, groupSize)
                    .replace(/&pp/g, ''); // &pp removed from text, image handled via preview

                // Send goodbye message with external preview
                await conn.sendMessage(groupJid, {
                    text,
                    mentions: [user],
                    contextInfo: {
